...
gen_figure_1 <- function(df,CTC_df){
  ...
  plt_1 <- ggplot(CTC_df,aes(x=Time_AKST,y=val,color = Species))+
    ...
    scale_y_continuous(name = expression(bold(SO[2]*' (ppbv)')),
                       ...
                       sec.axis = sec_axis(transform = ~., name=expression(bold('Temperature ('*degree*'C)')),
                                           breaks = seq(-40,55,10)))+
  ...
  plt_2 <- ggplot(df %>% filter(Species %in% panel_2),aes(x=Time_AKST,y=val,color = Species))+
    annotate("rect", xmin=cold_event[1], xmax=cold_event[2], ymin=0, ymax=650, alpha=0.3, fill='#1E90FF',color = '#1E90FF')+
    annotate("rect", xmin=warm_event[1], xmax=warm_event[2], ymin=0, ymax=650, alpha=0.2, fill='orange',color = 'orange')+
    geom_line(linewidth=1)+
  ...
  plt_3 <- ggplot(df %>% filter(Species %in% panel_3),aes(x=Time_AKST,y=val,color = Species))+
    ...
    scale_y_continuous(name = expression(bold(PM[1]*' ('*mu*'g '*m^-3*')')),
                       breaks = seq(0,100,20),minor_breaks = seq(0,100,5),limits = c(0,100),expand = c(0,0),
...
gen_figure_7 <- function(df){
  ...
  spec_list <- c('TNRM','SO4','NOx','CO','Formaldehyde','Formic_Acid','Acetic_Acid','Furfural','Furan','Toluene')
  df <- df %>% filter(Species_Name %in% spec_list)
  
  df <- df %>% mutate_at(vars(Species_Name),~gsub('_',' ',.))
  
  spec_labs <- c(expression(bold('Acetic Acid')),expression(bold('CO')),
                 expression(bold('Formaldehyde')),expression(bold('Formic Acid')),
                 expression(bold('Furan')),expression(bold('Furfural')),
                 expression(bold(NO[x])),expression(bold({SO[4]^{'2-'}})),
                 expression(bold('TNRM')),expression(bold('Toluene')))
